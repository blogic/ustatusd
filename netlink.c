...
static bool
nl_raw_status_socket(struct status_socket *ev, int protocol, int groups,
...
bool
nl_status_socket(struct status_socket *ev, int protocol,
		int (*cb)(struct nl_msg *msg, void *arg), void *priv)
{
	if (!nl_raw_status_socket(ev, protocol, 0, handler_nl_status, ULOOP_ERROR_CB))
		return false;
...
int genl_send_and_recv(struct status_socket *ev, struct nl_msg * msg)
{
